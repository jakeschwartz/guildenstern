...
export const Sheet = ({ open, onClose, title, children }: Props) => (
  <>
    ...
    />
    <div
      className={`absolute inset-x-0 bottom-0 z-40 bg-paper rounded-t-3xl border-t border-rule shadow-[0_-12px_40px_-12px_rgba(17,17,17,0.25)] transition-transform duration-200 ease-out ${
    ...
    >
      ...
      {title && (
        <div className="px-5 pb-2 text-[12px] text-muted">{title}</div>
      )}
